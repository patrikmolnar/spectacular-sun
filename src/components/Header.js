...
export default class Header extends React.Component {
    render() {
        ...
        let isHome = _.get(page, 'frontmatter.isHome') || false;
        return (
            <header className="header">
                <nav className={classNames('nav', {'nav--light': isHome, 'nav--dark': isHome !== true})}>
                    <div className="nav__logo">
                        <Link to="/">
                            {((isHome === true) || (_.get(page, 'frontmatter.template') === 'product')) ? 
                                <Picture {...this.props} image={_.get(site, 'siteMetadata.logos.light')} cssClass={'footer_logo nav__logo-image'} alt={'Planty logo'} />
                             : 
                                <Picture {...this.props} image={_.get(site, 'siteMetadata.logos.dark')} cssClass={'footer_logo nav__logo-image'} alt={'Planty logo'} />
                            }
                        </Link>
                    </div>
                    <ul className="nav__menu">
                        {_.map(_.get(this.props, 'pageContext.menus.main'), (item, item_idx) => {
                            let section = _.get(page, 'frontmatter.section') || _.get(page, 'frontmatter.title');
                            let isActive = (_.get(item, 'title') === section) ? true : false;
                            return (<React.Fragment key={item_idx}>
                                <li key={item_idx} className="nav__menu-item">
                                    <Link to={safePrefix(_.get(item, 'url'))} className={classNames('nav__menu-item-link', {'nav__menu-item-link--active': isActive})}>
                                        {_.get(item, 'title')}
                                    </Link>
                                </li>
                            </React.Fragment>)
                                ...
                                {_.map(_.get(this.props, 'pageContext.menus.main'), (item, item_idx) => {
                                    ...
                                    return (<React.Fragment key={item_idx}>
                                        <li key={item_idx} className="hamburger__nav-item">
                                            <Link to={safePrefix(_.get(item, 'url'))} className={classNames('hamburger__nav-link', {'hamburger__nav-link--active': isActive})}>
                                                {_.get(item, 'title')}
                                            </Link>
                                        </li>
                                    </React.Fragment>)
