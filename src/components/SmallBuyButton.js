...
export default class SmallBuyButton extends React.Component {
    render() {
        ...
        let component = _.get(this.props, 'component');
        return (
            <button className={component + '__item-buy button button--icon-only snipcart-add-item'}
                data-item-name={_.get(product_page, 'frontmatter.title')}
                data-item-url={_.get(product_page, 'url')}
                data-item-price={_.get(product_page, 'frontmatter.price')}
                dateItemCategories={_.get(product_page, 'frontmatter.category')}
                data-item-image={_.get(product_page, 'frontmatter.images.default.thumbnail.fallback')}
                data-item-description={_.get(product_page, 'frontmatter.description')}
                data-item-id={_.get(product_page, 'frontmatter.id')}>
